...
ExportLine('"qualities": [', 1)
if output.qualities ~= nil then
	counter = 0 for _,_ in pairs(output.qualities) do counter = counter + 1 end
	for _, data in pairs(output.qualities) do
		ExportLine('{', 2)
		ExportParameter('name', data.name, false, ',', 3)
		ExportParameter('icon_name', "icon.q."..data.name, false, ',', 3)
		ExportIcon(data.icon, data.icon_size, data.icons)
		counter = counter - 1 if counter > 0 then ExportLine('},', 2) else ExportLine('}', 2) end
	end
end
